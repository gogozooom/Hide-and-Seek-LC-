...
public static class Abilities
{
    ...
    static void CriticalInjuryServerEvent(AbilityBase ability, ulong activatorId)
    {
        ...
        {
            playerTarget = HideAndSeekGM.instance.GetPlayerWithClientId(activatorId).actualClientId;
    ...
    static void TauntClientEvent(AbilityBase ability, ulong activatorId, string extraMessage = null)
    {
        ...
        Vector3 playerPosition = HideAndSeekGM.instance.GetPlayerWithClientId(activatorId).transform.position;
    ...
    {
        HideAndSeekGM.instance.SpawnNewItem("Key", HideAndSeekGM.instance.GetPlayerWithClientId(activatorId), true);
    }
    static void SpawnTZPServerEvent(AbilityBase ability, ulong activatorId)
    {
        HideAndSeekGM.instance.SpawnNewItem("TZP-Inhalant", HideAndSeekGM.instance.GetPlayerWithClientId(activatorId), true);
    }
    static void SpawnFlashlightServerEvent(AbilityBase ability, ulong activatorId)
    {
        HideAndSeekGM.instance.SpawnNewItem("Flashlight", HideAndSeekGM.instance.GetPlayerWithClientId(activatorId), true);
    }
    static void SpawnWalkieServerEvent(AbilityBase ability, ulong activatorId)
    {
        HideAndSeekGM.instance.SpawnNewItem("Walkie-talkie", HideAndSeekGM.instance.GetPlayerWithClientId(activatorId), true);
    }
    static void SpawnShovelServerEvent(AbilityBase ability, ulong activatorId)
    {
        HideAndSeekGM.instance.SpawnNewItem("Shovel", HideAndSeekGM.instance.GetPlayerWithClientId(activatorId), true);
    }
    static void SpawnStunGServerEvent(AbilityBase ability, ulong activatorId)
    {
        HideAndSeekGM.instance.SpawnNewItem("Stun grenade", HideAndSeekGM.instance.GetPlayerWithClientId(activatorId), true);
    }
    static void SpawnRemoteServerEvent(AbilityBase ability, ulong activatorId)
    {
        HideAndSeekGM.instance.SpawnNewItem("Remote", HideAndSeekGM.instance.GetPlayerWithClientId(activatorId), true);
    }
    ...
    {
        Transform playerT = HideAndSeekGM.instance.GetPlayerWithClientId(activatorId).transform;
        Vector3 positionToSpawn = playerT.position + (playerT.forward * 1.3f);

        if (!turretPrefab)
        {
            if (!HideAndSeekGM.instance.currentLevel)
            {
                Debug.LogError("No current level loaded!");
                return;
            }

            foreach (var item in HideAndSeekGM.instance.currentLevel.spawnableMapObjects)
            {
    ...
    static IEnumerator SwapClientEventC(AbilityBase ability, ulong activatorId, string extraMessage = null)
    {
        ...
        PlayerControllerB localPlayer = GameNetworkManager.Instance.localPlayerController;
        PlayerControllerB activatorPlayer = HideAndSeekGM.instance.GetPlayerWithClientId(activatorId);
        PlayerControllerB targetPlayer = HideAndSeekGM.instance.GetPlayerWithClientId(targetPlayerId);
        ...
        if (activatorPlayer.isPlayerDead || targetPlayer.isPlayerDead)
        {
            ...
            {
                // User
                localPlayer.GetComponent<AbilityInstance>().DisplayTip($"Failed to switch places with '{HideAndSeekGM.instance.GetPlayerWithClientId(targetPlayerId).playerUsername}'", false);
            }
            yield break;
        }
    ...
    {
        PlayerControllerB localPlayer = HideAndSeekGM.instance.GetPlayerWithClientId(activatorId);
